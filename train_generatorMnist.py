...
def main():
    ...
    for epoch in np.arange(100):
        netG.train()
        print("epoch", epoch)
        for i, (batch_imgs, batch_lbls) in enumerate(trainloader):
            #print(1)
            ...
            errG = lambd1 * criterion(Z_bar, Z_origin) + lambd2 * criterion(X_bar, batch_imgs.cuda())
        ...
        if epoch > 5:
            ...
            print("-------------------------")
        if epoch > 10:
            #errE = lambd1 * criterion(Z_bar, Z_origin)
            #print(errE)
            show(X_show)
            show(X_bar_show)
