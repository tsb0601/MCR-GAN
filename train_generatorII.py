...
def main():
    ...
    for epoch in np.arange(100):
        ...
        for step, (batch_imgs, batch_lbls) in enumerate(trainloader):
            ...
            indices = batch_lbls == 1
            batch_imgs = batch_imgs[indices]
            batch_lbls = batch_lbls[indices]
            batch_size = len(batch_lbls)
            ...
            #step = vutils.make_grid(batch_imgs, normalize=True)

            #plt.imshow(vutils.make_grid(batch_imgs, normalize=True))

            #imX_bar = np.transpose(X_bar.detach().cpu().numpy()[0], (1, 2, 0))
            ...
            #plt.imshow(vutils.make_grid(imX_bar, padding=2, normalize=True))

            Z_bar = net(X_bar)

            #X_barbar = netG(torch.reshape(Z_bar, (batch_size, 128, 1, 1)))

            #errG = criterion(Z_origin, Z_bar)
            #errG = 6 * criterion(Z_bar, Z_origin) + lambd * criterion(batch_imgs.cuda(), X_barbar) + 0.7 * criterion(batch_imgs.cuda(), X_bar)
        ...
        errZ = criterion(Z_bar, Z_origin)
        print(f"L1 is error is {errG}")
        print(f"Z error is {errZ}")
        if epoch > 8:
            show(X_show)
            show(X_bar_show)
